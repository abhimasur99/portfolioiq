...
**[Checkpoint — Stage E COMPLETE (2026-03-20)]**: Grounded UX review & implementation. Changes: (EA) Removed Beta block from Q2 At a Glance insight_lines (Beta is Q1-only, not a Q2 KPI). (E1+E2) Time frame architecture overhaul — always fetch 5y data in data_fetcher.py (no period param); input.py stores SK_PRICE_DATA_FULL + slices default 3y window; dashboard.py adds _recompute_for_window(), _render_time_selector() (1Y/3Y/5Y/All/Custom radio + inline date pickers for Custom); context badge replaces freshness caption. (E3) DEFAULT_MC_HORIZON changed 10→1; settings MC horizon options [1,2,3,5]. (E4) Q3 charts stacked full-width; signal_scenario_chart() bars flipped downward (losses below zero), multiplier annotations added, text labels on bars. (E6) 60/40 benchmark removed from BENCHMARK_OPTIONS; D-11 added to DECISIONS.md. (E10) Rebalancing/drift threshold section removed from settings.py; PERIOD_OPTIONS and period selectbox removed from settings.py and input.py. New SK_ keys: SK_ANALYSIS_START, SK_ANALYSIS_END, SK_PRICE_DATA_FULL. CHANGELOG [0.22.0] added.

**[Checkpoint — Stage F COMPLETE (2026-03-20)]**: 6 bug fixes and UI polish. (F1) Rolling chart crash fix — adaptive window (`252 if n >= 253 else max(21, n // 3)`) in `compute_all_performance`; stores `rolling_sharpe_window` / `rolling_beta_window` in output dict; empty-series guard + window param added to `rolling_sharpe_chart` and `rolling_beta_chart`; disclosure caption shown in details_q1.py when window < 252. (F3) AGG bond ETF removed from BENCHMARK_OPTIONS — equities only; D-12 added to DECISIONS.md. (F4) MC fan chart base = actual portfolio dollar value — new `SK_TOTAL_VALUE` key stored in input.py; passed to `monte_carlo_fan_chart()` in dashboard.py and details_q3.py. (F5) Q3 More Details chart order: Signal Scenarios now above MC fan. (F6) Holdings strip redesigned — persistent label row below bar (color dot + ticker + weight%) replaces threshold-based inline labels; all positions always labeled. (F7) MC y-axis label → "Portfolio Value ($)"; signal scenario chart margin=dict(t=70). Time selector simplified to 1Y/3Y/5Y only (Custom/All removed). Tests updated (2 test fixes): `test_all_keys_present` adds 2 new keys; `test_network_failure_marks_all_invalid` renamed and updated to reflect pass-through behavior. All 151 tests passing. CHANGELOG [0.23.0] added.

**[Checkpoint — Stage G COMPLETE (2026-03-20)]**: 4 improvements. (G1) Q2 drawdown chart now overlays benchmark drawdown — `drawdown_chart()` gains optional `benchmark_drawdown`/`benchmark_label` params; details_q2.py computes benchmark drawdown and passes it; portfolio shown as red filled area, benchmark as gray dotted line. (G3) VaR method selector made functional — `SK_VAR_METHOD` now read in dashboard.py `_build_q3()`, details_q3.py Key Metrics, and details_q1.py return distribution chart; GARCH method shows `var_95_garch` as primary; CVaR always historical; settings help text expanded. (G4) GARCH chart — EWMA rolling series computed by `_compute_ewma_series_pct()` in risk_outlook.py; stored as `ewma_series` in analytics dict; shown as purple trace (COLOR_PURPLE) in `garch_volatility_chart()` on 1Y window, distinct from blue GARCH on 3Y/5Y. (G5) Signal Sensitivity Analysis upgraded: (a) Filtered Historical Simulation replaces normal-distribution VaR — scales empirical 5th-percentile by vol ratio, preserves fat tails/skew; (b) daily/monthly toggle radio (default monthly) above chart; (c) bars show combined "% / −$" labels when total_value available. All 151 tests passing. CHANGELOG [0.24.0] added.

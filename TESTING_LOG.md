...
**2026-03-20 — Stage E (Grounded UX Review)**
Items implemented: (EA) Q2 At a Glance Beta block removed — Beta is a Q1 metric, not Q2 KPI; details_q2.py insight_lines no longer has beta conditional. (E1+E2) Time frame architecture: data_fetcher.fetch_price_data() now always fetches 5y (period param removed, cache key = tickers-only); input.py stores full 5y prices in SK_PRICE_DATA_FULL, slices 3y default window, sets SK_ANALYSIS_START/END; period selectbox removed from input.py; dashboard.py adds _window_start(), _recompute_for_window(), _render_time_selector() (1Y/3Y/5Y/All/Custom radio + Custom date pickers); render() now shows context badge ("Xy Analysis | Benchmark: X | Data as of...") and time selector before health bar. (E3) DEFAULT_MC_HORIZON 10→1 in config.py; settings MC horizon options changed to [1,2,3,5]. (E4) details_q3.py chart layout changed from 2-column (MC fan + signal scenarios) to full-width stacking of all 3 charts; signal_scenario_chart() in charts.py: Y values negated (losses below zero), text labels on bars, multiplier annotations above bars, yaxis range anchored at 0. (E6) 60/40 benchmark removed from BENCHMARK_OPTIONS in config.py; D-11 added to DECISIONS.md. (E10) settings.py: removed Period section and period selectbox; removed Rebalancing/drift threshold section; MC horizon options [1,2,3,5]; imports cleaned (removed PERIOD_OPTIONS, DEFAULT_PERIOD, SK_DRIFT_THRESHOLD, DEFAULT_DRIFT_THRESHOLD); _snapshot/_restore_defaults updated. New SK_ keys in config.py: SK_ANALYSIS_START, SK_ANALYSIS_END, SK_PRICE_DATA_FULL, SK_PRICE_DATA. CHANGELOG [0.22.0].
